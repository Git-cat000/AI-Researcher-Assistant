...
详细技术设计和后续规划见 [TECHNICAL_DESIGN.zh-CN.md](TECHNICAL_DESIGN.zh-CN.md)，英文版见 [TECHNICAL_DESIGN.md](TECHNICAL_DESIGN.md)。
...
- 新增 `harness_coordination` 内置 Skill，用于持久任务图、团队邮箱、请求响应协议和 worktree 绑定元数据。
- 系统提示词改为轻量 Skill catalog，详细 Skill 知识通过 observation 按需加载，避免每轮上下文膨胀。
...
项目新增了受 `learn-claude-code` s07-s12 启发的本地协调控制面：

```text
.ai-researcher/
  tasks/                 持久任务 DAG
  team/inbox/            JSONL 团队邮箱
  team/protocols.json    request/response 协议状态
  worktrees/             任务与隔离目录的绑定索引和事件日志
```

`harness_coordination` Skill 支持创建任务、更新依赖、自动认领 ready task、发送和读取团队消息、创建和响应审批请求、记录任务与 worktree 的绑定关系。当前 worktree 层只记录隔离意图和生命周期事件，不直接执行破坏性 git 命令。
