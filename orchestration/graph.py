
"""
基于图的编排模块。
提供类似 LangGraph 的状态图编排能力，支持：
- 节点（Node）：执行特定逻辑（LLM 调用、技能执行等）
- 边（Edge）：连接节点，支持条件分支
- 状态（GraphState）：在节点间传递的共享状态
"""
from typing import (
    Dict, Any, Optional, List, Callable, Awaitable, Union, TypedDict,
    Literal, Set, Tuple, cast
)
from dataclasses import dataclass, field
from enum import Enum
import asyncio
import logging
from abc import ABC, abstractmethod

from ai_researcher_assistant.llm import BaseLLM
from ai_researcher_assistant.skills.registry import SkillRegistry, get_skill_registry
from ai_researcher_assistant.orchestration.state import ExecutionState, ExecutionStep
from ai_researcher_assistant.core.message import Conversation, MessageRole

logger = logging.getLogger(__name__)


class GraphState(TypedDict, total=False):
    """图状态字典，在节点间传递"""
    task: str
    conversation: Conversation
    execution_state: ExecutionState
    llm_response: str
    skill_result: Dict[str, Any]
    next_action: str
    error: Optional[str]
    metadata: Dict[str, Any]


NodeFunc = Callable[[GraphState], Awaitable[GraphState]]
ConditionalFunc = Callable[[GraphState], str]


class NodeType(str, Enum):
    """节点类型"""
    LLM = "llm"
    SKILL = "skill"
    FUNCTION = "function"
    CONDITION = "condition"
    START = "start"
    END = "end"


@dataclass
class Node:
    """图节点"""
    name: str
    func: NodeFunc
    node_type: NodeType = NodeType.FUNCTION
    description: str = ""
    metadata: Dict[str, Any] = field(default_factory=dict)


@dataclass
class Edge:
    """图边"""
    source: str
    target: str
    condition: Optional[ConditionalFunc] = None
    label: str = ""


class StateGraph:
    """
    状态图构建器与执行器。
    支持构建有向图并执行，节点可以是异步函数。
    """

    def __init__(self, name: str = "AgentGraph"):
        self.name = name
        self.nodes: Dict[str, Node] = {}
        self.edges: List[Edge] = []
        self.conditional_edges: Dict[str, List[Edge]] = {}
        self.entry_point: Optional[str] = None
        self.finish_points: Set[str] = set()

    def add_node(
        self,
        name: str,
        func: NodeFunc,
        node_type: NodeType = NodeType.FUNCTION,
        description: str = "",
        **metadata
    ) -> "StateGraph":
        """添加节点"""
        self.nodes[name] = Node(
            name=name,
            func=func,
            node_type=node_type,
            description=description,
            metadata=metadata,
        )
        return self

    def add_llm_node(
        self,
        name: str,
        llm: BaseLLM,
        prompt_template: str,
        **metadata
    ) -> "StateGraph":
        """便捷添加 LLM 节点"""
        async def llm_node(state: GraphState) -> GraphState:
            prompt = prompt_template.format(**state)
            messages = [{"role": "user", "content": prompt}]
            response = await llm.agenerate(messages)
            state["llm_response"] = response.content
            return state

        return self.add_node(name, llm_node, NodeType.LLM, **metadata)

    def add_skill_node(
        self,
        name: str,
        skill_name: str,
        skill_registry: Optional[SkillRegistry] = None,
        param_mapping: Optional[Callable[[GraphState], Dict[str, Any]]] = None,
        **metadata
    ) -> "StateGraph":
        """便捷添加技能执行节点"""
        registry = skill_registry or get_skill_registry()

        async def skill_node(state: GraphState) -> GraphState:
            if param_mapping:
                params = param_mapping(state)
            else:
                params = state.get("skill_parameters", {})
            result = await registry.aexecute(skill_name, params, {"state": state})
            state["skill_result"] = result
            if not result.get("success"):
                state["error"] = result.get("error")
            return state

        return self.add_node(name, skill_node, NodeType.SKILL, **metadata)

    def set_entry_point(self, node_name: str) -> "StateGraph":
        """设置入口节点"""
        if node_name not in self.nodes:
            raise ValueError(f"Node '{node_name}' not found")
        self.entry_point = node_name
        return self

    def set_finish_point(self, node_name: str) -> "StateGraph":
        """标记为结束节点"""
        if node_name not in self.nodes:
            raise ValueError(f"Node '{node_name}' not found")
        self.finish_points.add(node_name)
        return self

    def add_edge(self, source: str, target: str, label: str = "") -> "StateGraph":
        """添加普通边"""
        if source not in self.nodes:
            raise ValueError(f"Source node '{source}' not found")
        if target not in self.nodes:
            raise ValueError(f"Target node '{target}' not found")
        self.edges.append(Edge(source=source, target=target, label=label))
        return self

    def add_conditional_edges(
        self,
        source: str,
        condition: ConditionalFunc,
        targets: Dict[str, str],
        label: str = ""
    ) -> "StateGraph":
        """添加条件边，根据 condition 返回值选择目标节点"""
        if source not in self.nodes:
            raise ValueError(f"Source node '{source}' not found")
        for t in targets.values():
            if t not in self.nodes:
                raise ValueError(f"Target node '{t}' not found")
        # 存储为多个条件边，执行时计算
        self.conditional_edges[source] = [
            Edge(source=source, target=targets[k], condition=condition, label=f"{label}:{k}")
            for k in targets
        ]
        return self

    def compile(self) -> "CompiledGraph":
        """编译图，返回可执行对象"""
        # 验证图结构
        if self.entry_point is None:
            raise ValueError("Entry point not set")
        # 检查是否存在从入口到某个结束节点的路径（简单检查）
        return CompiledGraph(
            name=self.name,
            nodes=self.nodes,
            edges=self.edges,
            conditional_edges=self.conditional_edges,
            entry_point=self.entry_point,
            finish_points=self.finish_points,
        )


class CompiledGraph:
    """编译后的可执行图"""

    def __init__(
        self,
        name: str,
        nodes: Dict[str, Node],
        edges: List[Edge],
        conditional_edges: Dict[str, List[Edge]],
        entry_point: str,
        finish_points: Set[str],
    ):
        self.name = name
        self.nodes = nodes
        self.edges = edges
        self.conditional_edges = conditional_edges
        self.entry_point = entry_point
        self.finish_points = finish_points

        # 构建邻接表加速路由
        self._build_adjacency()

    def _build_adjacency(self):
        """构建邻接关系"""
        self.adjacency: Dict[str, List[Edge]] = {name: [] for name in self.nodes}
        for edge in self.edges:
            self.adjacency[edge.source].append(edge)
        for source, edges in self.conditional_edges.items():
            self.adjacency[source].extend(edges)

    async def run(self, initial_state: GraphState, max_steps: int = 50) -> GraphState:
        """
        执行图。
        
        Args:
            initial_state: 初始状态
            max_steps: 最大执行步数
            
        Returns:
            最终状态
        """
        current_node = self.entry_point
        state = initial_state.copy()
        step_count = 0

        # 初始化执行状态记录（如果存在）
        if "execution_state" in state:
            exec_state = state["execution_state"]
        else:
            from ai_researcher_assistant.orchestration.state import ExecutionState
            exec_state = ExecutionState()
            exec_state.start_task(state.get("task", "Graph execution"))
            state["execution_state"] = exec_state

        logger.info(f"Starting graph '{self.name}' execution from node '{current_node}'")

        while current_node and step_count < max_steps:
            if current_node in self.finish_points:
                logger.info(f"Reached finish node '{current_node}'")
                break

            if current_node not in self.nodes:
                logger.error(f"Node '{current_node}' not found")
                state["error"] = f"Node '{current_node}' not found"
                break

            node = self.nodes[current_node]
            logger.debug(f"Executing node '{current_node}' (type: {node.node_type})")

            try:
                # 记录步骤开始
                step = ExecutionStep(thought=f"Executing graph node: {current_node}")
                exec_state.add_step(step)

                # 执行节点
                state = await node.func(state)

                # 更新步骤
                step.observation = f"Node '{current_node}' completed"
                if "error" in state and state["error"]:
                    step.error = state["error"]

                # 选择下一个节点
                next_node = await self._get_next_node(current_node, state)
                current_node = next_node

            except Exception as e:
                logger.exception(f"Error executing node '{current_node}'")
                state["error"] = str(e)
                step.error = str(e)
                break

            step_count += 1

        if step_count >= max_steps:
            state["error"] = "Max steps exceeded"
            exec_state.fail("Max steps exceeded")
        elif "error" not in state or not state["error"]:
            exec_state.complete("Graph execution completed")

        return state

    async def _get_next_node(self, current: str, state: GraphState) -> Optional[str]:
        """根据当前节点和状态确定下一个节点"""
        edges = self.adjacency.get(current, [])

        for edge in edges:
            if edge.condition:
                # 条件边：执行条件函数获取目标
                target_key = edge.condition(state)
                # 寻找匹配的目标（条件边以 label 区分）
                for e in self.conditional_edges.get(current, []):
                    if e.label.endswith(f":{target_key}") or e.label == target_key:
                        return e.target
            else:
                # 无条件边，直接返回目标
                return edge.target

        return None


class AgentGraphBuilder:
    """
    针对 ResearcherAgent 的图构建器。
    提供预定义的常用图模式。
    """

    def __init__(self, llm: BaseLLM, skill_registry: Optional[SkillRegistry] = None):
        self.llm = llm
        self.skill_registry = skill_registry or get_skill_registry()

    def build_react_graph(self) -> StateGraph:
        """
        构建 ReAct 模式的图。
        
        结构：
        START -> think -> [condition] -> act -> observe -> think ...
                                    |-> final_answer -> END
        """
        graph = StateGraph("ReActGraph")

        # 思考节点
        async def think_node(state: GraphState) -> GraphState:
            conv = state.get("conversation")
            if not conv:
                conv = Conversation()
                conv.add_system("You are a ReAct agent...")
                state["conversation"] = conv

            messages = conv.get_context_window()
            response = await self.llm.agenerate(messages, temperature=0.0)
            state["llm_response"] = response.content

            # 解析 thought 和 action
            import re
            text = response.content
            thought_match = re.search(r"Thought:\s*(.+?)(?=Action:|$)", text, re.DOTALL)
            state["thought"] = thought_match.group(1).strip() if thought_match else text

            action_match = re.search(r"```json\s*(\{.*?\})\s*```", text, re.DOTALL)
            if action_match:
                import json
                try:
                    state["action"] = json.loads(action_match.group(1))
                except:
                    state["action"] = None
            else:
                state["action"] = None

            # 检查是否是最终答案
            if "Final Answer:" in text:
                state["is_final"] = True
                final_match = re.search(r"Final Answer:\s*(.+)$", text, re.DOTALL)
                state["final_answer"] = final_match.group(1).strip() if final_match else text
            else:
                state["is_final"] = False

            return state

        # 条件路由函数
        def route_after_think(state: GraphState) -> str:
            if state.get("is_final"):
                return "final"
            elif state.get("action"):
                return "act"
            else:
                return "think"  # 重新思考

        # 行动节点
        async def act_node(state: GraphState) -> GraphState:
            action = state.get("action", {})
            skill_name = action.get("skill")
            parameters = action.get("parameters", {})

            if skill_name:
                result = await self.skill_registry.aexecute(skill_name, parameters, {"state": state})
                state["skill_result"] = result
                # 格式化观察
                if result.get("success"):
                    state["observation"] = f"Skill '{skill_name}' executed successfully."
                else:
                    state["observation"] = f"Error: {result.get('error')}"
            else:
                state["observation"] = "No skill specified."

            # 将行动和观察添加到对话历史
            conv = state["conversation"]
            conv.add(MessageRole.ASSISTANT, f"Thought: {state.get('thought')}\nAction: {json.dumps(action)}")
            conv.add(MessageRole.OBSERVATION, f"Observation: {state.get('observation')}")
            return state

        # 最终答案节点
        async def final_node(state: GraphState) -> GraphState:
            # 只需传递状态
            return state

        graph.add_node("think", think_node, NodeType.LLM)
        graph.add_node("act", act_node, NodeType.SKILL)
        graph.add_node("final", final_node, NodeType.END)

        graph.set_entry_point("think")
        graph.set_finish_point("final")

        graph.add_conditional_edges("think", route_after_think, {"act": "act", "final": "final"})
        graph.add_edge("act", "think")  # 执行后返回思考

        return graph

    def build_research_agent_graph(self) -> StateGraph:
        """
        构建学术研究 Agent 的专用图。
        
        流程：
        START -> analyze_task -> search_papers -> read_paper -> synthesize -> END
                                     |              |
                                     +--------------+ (可循环)
        """
        graph = StateGraph("ResearchAgentGraph")

        # 任务分析节点
        async def analyze_task(state: GraphState) -> GraphState:
            task = state.get("task", "")
            prompt = f"""Analyze the following research task and extract key information:
Task: {task}

Output a JSON with:
- "query": search query for arXiv
- "categories": list of relevant arXiv categories
- "max_papers": number of papers to retrieve
"""
            response = await self.llm.agenerate([{"role": "user", "content": prompt}], temperature=0.0)
            import json, re
            text = response.content
            match = re.search(r"```json\s*(\{.*?\})\s*```", text, re.DOTALL)
            if match:
                state["analysis"] = json.loads(match.group(1))
            else:
                state["analysis"] = {"query": task, "categories": ["hep-th"], "max_papers": 5}
            return state

        # 搜索论文节点
        async def search_papers(state: GraphState) -> GraphState:
            analysis = state.get("analysis", {})
            params = {
                "query": analysis.get("query", ""),
                "categories": analysis.get("categories", ["hep-th"]),
                "max_results": analysis.get("max_papers", 5),
            }
            result = await self.skill_registry.aexecute("arxiv_fetcher", params, {"state": state})
            state["search_result"] = result
            if result.get("success"):
                papers = result.get("result", {}).get("papers", [])
                state["papers"] = papers
                state["current_paper_index"] = 0
            return state

        # 阅读论文节点
        async def read_paper(state: GraphState) -> GraphState:
            papers = state.get("papers", [])
            idx = state.get("current_paper_index", 0)
            if idx < len(papers):
                paper = papers[idx]
                # 读取论文（通过 URL）
                result = await self.skill_registry.aexecute(
                    "paper_reader",
                    {"url": paper.get("pdf_url")},
                    {"state": state}
                )
                if "paper_contents" not in state:
                    state["paper_contents"] = []
                state["paper_contents"].append({
                    "paper": paper,
                    "content": result.get("result", {}).get("full_text", "") if result.get("success") else "",
                })
                state["current_paper_index"] = idx + 1
            return state

        # 条件：是否还有论文需要阅读
        def should_read_more(state: GraphState) -> str:
            idx = state.get("current_paper_index", 0)
            papers = state.get("papers", [])
            if idx < len(papers):
                return "read"
            return "synthesize"

        # 综合节点
        async def synthesize(state: GraphState) -> GraphState:
            task = state.get("task", "")
            papers = state.get("papers", [])
            contents = state.get("paper_contents", [])

            prompt = f"""Based on the research task: "{task}"

You have retrieved the following papers:
{json.dumps([{"title": p.get("title"), "abstract": p.get("abstract")} for p in papers], indent=2)}

And read their contents.

Please provide a comprehensive answer to the research question. Include:
- Summary of relevant findings
- Key methodologies
- Citations (arXiv IDs)
- Potential future directions
"""
            response = await self.llm.agenerate([{"role": "user", "content": prompt}], temperature=0.3)
            state["final_answer"] = response.content
            return state

        graph.add_node("analyze", analyze_task, NodeType.LLM)
        graph.add_node("search", search_papers, NodeType.SKILL)
        graph.add_node("read", read_paper, NodeType.SKILL)
        graph.add_node("synthesize", synthesize, NodeType.LLM)

        graph.set_entry_point("analyze")
        graph.set_finish_point("synthesize")

        graph.add_edge("analyze", "search")
        graph.add_edge("search", "read")
        graph.add_conditional_edges("read", should_read_more, {"read": "read", "synthesize": "synthesize"})

        return graph
